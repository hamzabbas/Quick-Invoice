...
document.addEventListener('DOMContentLoaded', (event) => {
    ...
    <script type="module">
        $(document).ready(function() {
            ...
            const existingPayer = "{{ $invoice->booking_payer }}";

// AJAX request to fetch payers
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        payerDropdown.append('<option disabled>Select Payer</option>');
                        
                        // Loop through the response data and add options to the dropdown
                        $.each(response, function(key, payer) {
                            // Check if the payer matches the existing payer and set it as selected
                            const selected = payer.name === existingPayer ? 'selected' : '';
                            payerDropdown.append('<option value="' + payer.name + '" ' + selected + '>' + payer.name + '</option>');
                        });
                    },
                    error: function() {
